...
// For background processes' histogram
	...
	if(std::string(hist->GetName()) == "cutflow") {
            ...
            stack->Add(hist);
            legend->AddEntry(hist, histPair.second.legendName.c_str(), "f"); // "f"는 채워진 범례를 의미
        }
        else{
            maxY = max(maxY, hist->GetMaximum());
            hist->SetFillColor(histPair.second.color);
    ...
    for (const auto& histPair : histograms) {
        ...
        if (histPair.second.isSignal) {
            ...
            if(std::string(signalHist->GetName()) == "cutflow") {
                ...
                legend->AddEntry(signalHist, histPair.second.legendName.c_str(), "f"); // "f"는 채워진 범례를 의미
            }
            else{
                maxY = max(maxY, signalHist->GetMaximum());
	        signalHist->SetLineWidth(3);
...
int main() {
	...
	{"ttZtobb", {"ttZtobb.root", false, 9, "ttZtobb"}},
        ...
        {"ttTohadronic", {"ttTohadronic.root", false, 3, "tt to hadronic"}}
    ...
    unordered_map<string, vector<HistogramSetting>> histogramSettings = {
        ...
        {"baselineHistos", 
            {
                ...
                {"lightjetHT"     , "light jets HT", "HT [GeV]"},
        ...
        {"channelChi2ZZ", 
            {
                {"hchi2ZZ"        , "Reconstructed Z pair #chi^{2}", "#chi^{2}"},
                {"hmZ1cnd"        , "mass of 1st Z candidate", "invariant mass [GeV]"},
                {"hmZ2cnd"        , "mass of 2nd Z candidate", "invariant mass [GeV]"},

            }
	},
        {"channelChi2ZH", 
            {
                {"hchi2ZH"       , "Reconstructed Z pair #chi^{2}", "#chi^{2}"},
                {"hmHcnd"        , "mass of H candidate", "invariant mass [GeV]"},
                {"hmZcnd"        , "mass of Z candidate", "invariant mass [GeV]"},
